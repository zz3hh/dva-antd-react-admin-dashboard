    ...
    return (
      <div className={styles.main}>
        <Form onSubmit={this.handleSubmit}>
          <Tabs animated={false} className={styles.tabs} activeKey={type} onChange={this.onSwitch}>
            ...
            <TabPane tab="手机号码登录" key="mobile">
              {
                login.status === 'error' && login.type === 'mobile' && login.submmitting === false && this.renderMessage('验证码错误')
              }
              <FormItem>
                {getFieldDecorator('mobile', {
                  rules: [{
                    required: type === 'mobile', message: '请输入手机号码',
                  }, {
                    pattern: /^1\d{10}$/, message: '手机号码格式错误',
                  }],
                })(
                  <Input size="large" prefix={<Icon type="mobile" className={styles.prefixIcon} placeholder="手机号" />} />
                  )}
              </FormItem>
              <FormItem>
                <Row gutter={8}>
          ...
          <FormItem className={styles.additional}>
            ...
            <Button size="large" loading={login.submmitting} className={styles.submit} type="primary" htmlType="submit">登录</Button>
          </FormItem>
        </Form>
        <div className={styles.other}>
          ...
          <Link className={styles.register} to="/user/register">注册账号</Link>
        </div>
      </div>
    );
  }
